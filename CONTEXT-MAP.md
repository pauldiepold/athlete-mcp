...
- **Identität liegt im KV, Fachdaten liegen in D1.** Eine tragende Linie durchs ganze System: *wer ist wer und womit darf er wo rein* (`google:<sub>`, `apple:<sub>`, `invite:<code>`, `user:<id>:profile`, `user:<id>:garmin`, `user:<id>:finalsurge`, dazu die Fehler-Marker `user:<id>:<quelle>:fehler`, der Zustand der Erstbefüllung `user:<id>:garmin:erstbefuellung` und der kurzlebige `garmin:mfa:<handle>`) steht im KV; *was hat er* (`koerperdaten`, `steuerungsplan`, `steuerung_woche`) steht in D1. Die Trennung hat die OAuth-Umstellung überlebt, ohne dass eine Zeile in D1 gewandert wäre: Das Identitäts-Mapping ersetzt im KV genau die Stelle, die `pathsecret:` freimacht.
...
**Einrichtung**:
Die vier Schritte, die ein neues Konto vom Login bis zum ersten Steuerungsplan führt: Garmin verbinden, Final Surge verbinden, Connector hinzufügen, Onboarding in Claude starten. Die ersten beiden sind **überspringbar**, der dritte ist Pflicht, der vierte verlässt die Weboberfläche (und ist Pflicht, weil sein Ergebnis — der Steuerungsplan — das ist, wofür es das System gibt). Ihr Fortschritt wird durchgehend **abgeleitet** — Verbindungen aus dem KV, der Connector aus den Grants des Athleten (`listUserGrants`), das Onboarding aus dem Vorhandensein des Steuerungsplans; nichts davon wird gemeldet oder abgehakt. Reihenfolge und Pflicht liegen als reine Funktion in `web/shared/einrichtung.ts`, die Texte in `EinrichtungKarte.vue`. Solange ein **Pflicht**-Schritt offen ist, steht sie auf `/` an der Stelle des Dashboards und ersetzt es ganz; danach bleibt sie unter `/einstellungen` erreichbar — wer Final Surge nachreichen will, findet den Weg dort. Dass nur Pflichtschritte über dieses Ablösen entscheiden, ist der Sinn des Überspringens: Ein selbstgesteuerter Läufer ohne Coach hätte sonst dauerhaft eine unfertige Liste vor dem Dashboard.
_Avoid_: Wizard, Assistent (es gibt keine erzwungene Reihenfolge), Setup

**Erstbefüllung**:
Der Abruf der letzten 30 Tage Körperdaten, der nach dem Herstellen der Garmin-Verbindung im Hintergrund läuft. Gehört dem Garmin-Kontext und ist [dort](./src/garmin/CONTEXT.md) definiert; hier steht er nur, weil das *Einrichten* ihn auslöst.

**Startseiten-Zustand**:
Was ein angemeldeter Athlet auf `/` sieht, entschieden an **vorhandenen Körperdaten** statt am Verbindungszustand — und davor an der *Einrichtung*: *Einrichtung* (ein Pflichtschritt fehlt noch; steht vor allen anderen Fällen, auch vor einem laufenden Abruf und vorhandenen Daten, denn ohne Connector kommt Claude gar nicht an sie), *nicht verbunden* (Weg in die Einstellungen), *läuft* (die Erstbefüllung holt gerade — mit Ladehinweis und ausdrücklich **ohne** Knopf, damit niemand in einen laufenden Abruf hineindrückt), *keine Daten* (verbunden, kein Lauf — der Knopf zum Nachholen) und *Daten* (die Verläufe) — das Dashboard also erst, wenn alle Pflichtschritte stehen **und** Körperdaten da sind. Zwischen „verbunden" und „hat Daten" liegt die *Erstbefüllung*; am Verbinden aufgehängt zeigte die Seite direkt danach ein leeres Dashboard, und das sieht aus wie ein kaputtes. Vorhandene Tage werden auch während eines Laufs schon gezeigt. Die fünf Fälle liegen als reine Funktion in `web/shared/startseitenZustand.ts` — die **Reihenfolge** der Fälle ist die eigentliche Entscheidung. Die Seite fragt in den datenlosen Zuständen in Abständen nach und zieht bei jedem Wechsel die Verläufe und den Verbindungs-Hinweis mit; niemand muss neu laden. Die *Steuerung* bleibt aus jedem von ihnen erreichbar.
